...
import { automatedDesktop, emusim, liveDesktopWeb, nonAppInfo, rdcInfo } from './metadata.js'
...
async function getJobDetails(jobId = null, region, creds) {
    ...
    if (blob["application_summary"] !== null && blob.hasOwnProperty("application_summary")) {
        console.log("application_summary found, flipping app bit")
    ...
    if (blob["automation_backend"] === "espresso") {
        console.log("espresso test, flipping app bit")
        jobFlags |= JOB_TYPE.APP
    }

    console.log("cur_bitflag: ", jobFlags.toString(2))
